...
int length(node* linked_list)
{
    ...
    // pass a struct into a function (Pass it in by reference hence the use of the arrow)
    // by reference means that you can change them 
    // by value means that you cannot change them 
